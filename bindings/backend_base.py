...
Most backends will want to avoid the hooks and just use template rendering directly.
All files in a templates/ directory of the backend will be passed through Jinja2 with a top-level view of the metadata.
The hooks exist primarily for backends like java, which need to write one class per file.
...
"""
    ...
    def __init__(self, metadata, backend_path):
        ...
        self.jinja2_filters = {i[len("jfilt_"):]: getattr(self, i) for i in dir(self) if i.startswith("jfilt_")}
        self.jinja2_functions = {i[len("jfn_"):]: getattr(self, i) for i in dir(self) if i.startswith("jfn_")}
        self.jinja2_environment = jinja2.Environment(trim_blocks = True,
            loader = jinja2.FileSystemLoader(searchpath = backend_path))
        self.jinja2_environment.filters.update(self.jinja2_filters)
        ...
        if os.path.exists(os.path.join(backend_path, "templates")):
            self.prepopulate("templates", render = True)
...
If source is a directory, then the whole directory's contents will be copied to destination, including empty directories."""
    ...
    def prepopulate(self, path, render = False):
...
If render is True, pass the files through Jinja2.  The context will contain all public top-level attributes off the class metadata_description.Metadata.
...
Do not use this on a directory containing binary files.  See the class docstring for more details."""
        ...
        context = {i: getattr(self.metadata, i) for i in dir(self.metadata) if not i.startswith("_")}
        def render(text):
        ...
        for (dirpath, dirname, files) in os.walk(abspath):
            for p in [os.path.join(dirpath, j) for j in files]:
                ...
                builder.append_file(p, _preprocess_callback = render)
    ...
    def render(self, text, **kwargs):
        """Render a Jinja2 template, with kwargs as the globals.

This function inserts the Jinja2 functions into the globals in such a way that attributes you provide take priority. Try to avoid name clashes."""
        globals = dict(self.jinja2_functions)
        globals.update(kwargs)
        template = self.jinja2_environment.from_string(text, globals = globals)
...
As a convenience, lines can be appended with +=.  If you append more than one line, all lines are indented to the current level."""
    ...
    def append_file(self, path, _preprocess_callback = lambda x: x):
        with open(path) as f:
            text = f.read()
            text = _preprocess_callback(text)
            lines = text.split("\n")
            for i in lines:
