...
class LavDelayObject: public LavObject {
	public:
	LavDelayObject(std::shared_ptr<LavSimulation> simulation, float maxDelay);
	~LavDelayObject();
	void process();
	protected:
	void delayChanged();
	...
	float* line = nullptr;
	unsigned int read_head = 0, new_read_head = 0, write_head = 0;
	float read_head_weight = 1.0f, new_read_head_weight = 0.0f, delta = 1.0f;
	bool interpolating = false;
};

LavDelayObject::LavDelayObject(std::shared_ptr<LavSimulation> simulation, float maxDelay): LavObject(Lav_OBJTYPE_DELAY, simulation, 1, 1) {
	...
	line = new float[delay_line_length]();
...
std::shared_ptr<LavObject> createDelayObject(std::shared_ptr<LavSimulation> simulation, float maxDelay) {
	auto tmp = std::make_shared<LavDelayObject>(simulation, maxDelay);
...
void LavDelayObject::process() {
	...
	for(unsigned int i = 0; i < block_size; i++) {
		float outSample = line[read_head]*read_head_weight+line[new_read_head]*new_read_head_weight;
		...
		//write with feeback.
		line[write_head] = inputs[0][i]+outSample*feedback;
	...
	outputs[0][i] = outSample;
	}
}

//begin public api
Lav_PUBLIC_FUNCTION LavError Lav_createDelayObject(LavSimulation* simulation, float maxDelay, LavObject** destination) {
	PUB_BEGIN
	auto d = createDelayObject(incomingPointer<LavSimulation>(simulation), maxDelay);
