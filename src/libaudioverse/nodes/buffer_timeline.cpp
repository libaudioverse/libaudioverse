...
Lav_PUBLIC_FUNCTION LavError Lav_bufferTimelineNodeScheduleBuffer(LavHandle nodeHandle, LavHandle bufferHandle, double time, float pitchBend) {
	...
	if(n->getType() !=Lav_OBJTYPE_BUFFER_TIMELINE_NODE || b->getType() !=Lav_OBJTYPE_BUFFER) throw LavErrorException(Lav_ERROR_TYPE_MISMATCH);
	if(time < 0.0) throw LavErrorException(Lav_ERROR_RANGE);
	if(pitchBend <0.0) throw LavErrorException(Lav_ERROR_RANGE);
	LOCK(*n);
