...
float dotKernelSimple(int length, float* v1, float* v2) {
	float retval=0.0f;
...
float dotKernel(int length, float* v1, float* v2) {
	...
	for(int i= 0; i < length/4*4; i+=4) {
		accum = _mm_add_ps(accum, _mm_add_ps(_mm_load_ps(v1+i), _mm_load_ps(v2+i)));
	}
	...
	accum=_mm_add_ps(accum, _mm_shuffle_ps(accum, accum, 1));
	//extract.
