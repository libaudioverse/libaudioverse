...
Lav_PUBLIC_FUNCTION LavError Lav_automationEnvelope(LavHandle nodeHandle, int slot, double time, double duration, int valuesLength, double *values) {
	PUB_BEGIN
	//preconditions first.
	if(valuesLength ==0) throw LavErrorException(Lav_ERROR_RANGE);
	if(values == nullptr) throw LavErrorException(Lav_ERROR_RANGE);
	if(duration <= 0.0) throw LavErrorException(Lav_ERROR_RANGE);
